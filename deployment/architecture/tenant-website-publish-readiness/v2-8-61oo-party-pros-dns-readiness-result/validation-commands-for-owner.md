...
```powershell
az network dns zone show --resource-group rg-pumpkin-api-prod-centralus --name partyrentalphiladelphia.com --query "{name:name, nameServers:nameServers, numberOfRecordSets:numberOfRecordSets}" -o json
az network dns record-set a show --resource-group rg-pumpkin-api-prod-centralus --zone-name partyrentalphiladelphia.com --name "@" -o json

# Next Phase 2F-6 Encrypted Escrow Prototype Prompt

Approve Phase 2F-6 local encrypted secret escrow prototype only: use the Phase 2F-5 restore validation dry-run and hardened standard backup validator to design and implement a local-only encrypted escrow package prototype with fake secret fixtures only, deterministic test keys only, explicit no-real-secret boundaries, validation reports, and dry-run restore eligibility checks.

No real secret export, no protected config reads, no production key material, no real encrypted escrow payload for production, no real database export/import, no CMS/API calls, no real restore, no Azure/Cloudflare/DNS/deployment/email/Search Console actions, no external checks, and no live-page publication.

Required gate:

- Use fake secret fixtures only.
- Keep generated encrypted test payloads under ignored `.tmp`.
- Prove standard backups still exclude escrow.
- Prove escrow restore remains dry-run only.
- Produce JSON and Markdown validation reports.
- Do not proceed to real IceSkatingRinkRentals.com backup proof without separate owner approval.

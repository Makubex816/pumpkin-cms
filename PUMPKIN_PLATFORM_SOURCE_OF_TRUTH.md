...
| --- | --- |
| Current V2 reference | V2.8.14A, Staging Target Isolation And Deployment Auth Closure |
| Current V2 status | Complete; isolated non-production SWA target created and verified, artifact/tooling validation passes, deployment remains blocked only by missing `SWA_CLI_DEPLOYMENT_TOKEN` |
...
| Safety posture | V2.8.14A performed safe Azure discovery and exactly one approved isolated non-production SWA creation; no static deployment, DNS change, custom domain mutation, indexing, live publication, CMS write, provider write, app settings mutation, RBAC assignment, protected config read, deployment-token print/export/listing, keys/listKeys, connection string, SAS, contact form submission, contact endpoint POST, external crawl, or outbound URL check |
| Next gate | V2.8.14B Scoped Ice Isolated Staging Deployment Execution |
...
Approve V2.8.14B scoped Ice isolated staging deployment execution only: deploy the validated Ice static artifact to `swa-ice-static-isolated-staging` only if `SWA_CLI_DEPLOYMENT_TOKEN` is present in the same terminal session by presence-only check. Use the pinned repo-supported SWA CLI path, do not print or pass token values on the command line, perform bounded post-deploy checks only on `https://kind-island-0a85a740f.7.azurestaticapps.net/`, `/service-areas`, `/contact`, `/sitemap.xml`, and `/robots.txt`, and document result evidence. No production deployment, no deployment to `swa-ice-static-staging`, no DNS changes, no custom domain changes, no Search Console/indexing, no live-page publication, no external crawling beyond bounded Azure default-host route checks, no outbound URL checks, no contact form submission, no POST to contact endpoint, no CMS writes, no provider writes, no app settings mutation, no RBAC assignment, no protected config reads, no deployment-token print/export/listing, no Key Vault secrets, no keys/listKeys, no connection strings, no SAS, and no generated artifacts staged into Git.

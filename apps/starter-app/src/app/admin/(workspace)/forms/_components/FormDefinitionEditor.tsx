  ...
  const save = async () => {
    ...
    try {
      ...
      const url = mode === 'create'
        ...
        : `/api/admin/forms/definitions/${encodeURIComponent(getFormDefinitionId(initialDefinition))}`;
      ...
      router.replace(`/admin/forms/${encodeURIComponent(getFormDefinitionId(saved))}`);
      router.refresh();
    } catch (err) {
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-lg border border-neutral-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          ...
          <TextField label="Form ID" value={getFormDefinitionId(definition)} onChange={(value) => update('formDefinitionId', value)} disabled={mode === 'edit'} />
...
export function createFormDefinition(tenantId: string): FormDefinition {
  ...
  return normalizeFormDefinition({
    ...
    runtimeSubmitPath: `/api/forms/${tenantId}/submit/new_form`,
    staticEndpointRef: 'STARTER_FORM_ENDPOINT',
    leadRecipientRef: 'STARTER_LEAD_RECIPIENT',
    fields: [createField('name', 1), createField('email', 2, 'email')],
    hiddenFields: [],
    submitButtonText: 'Submit',
    successMessage: 'Thanks, your message was sent.',
    errorMessage: 'Unable to submit this form right now.',
    ...
    routing: {
      ...
      recipientGroupRef: 'STARTER_LEAD_RECIPIENT',
    },
    isActive: true,
    createdAt: now,
    updatedAt: now,
    createdBy: 'starter-admin',
    updatedBy: 'starter-admin',
  });
}

function normalizeFormDefinition(definition: FormDefinition): FormDefinition {
  const now = new Date().toISOString();
  const formKey = definition.formKey || definition.formDefinitionId || definition.id || 'new-form';
  const formType = definition.formType || 'custom';
  const activeType = definition.type || formType.replace(/-/g, '_');
  ...
  return {
    ...definition,
    id: definition.id || formKey,
    formDefinitionId: definition.formDefinitionId || definition.id || formKey,
    name: definition.name || 'New Form',
    tenantId: definition.tenantId || 'starter-tenant',
    siteKey: definition.siteKey || definition.tenantId || 'starter-tenant',
    formKey,
    type: activeType,
    description: definition.description || '',
    status: definition.status || (definition.isActive === false ? 'archived' : 'draft'),
    formType,
    version: definition.version || 'starter.v1',
    submitAction: definition.submitAction || 'form-entry',
    runtimeSubmitPath: definition.runtimeSubmitPath || `/api/forms/${definition.tenantId || 'starter-tenant'}/submit/${activeType}`,
    staticEndpointRef: definition.staticEndpointRef || 'STARTER_FORM_ENDPOINT',
    leadRecipientRef: definition.leadRecipientRef || 'STARTER_LEAD_RECIPIENT',
    ...
    errorMessage: definition.errorMessage || 'Unable to submit this form right now.',
    ...
    consent: definition.consent || {
      required: definition.spamProtection?.requireConsent ?? false,
      fieldName: definition.spamProtection?.consentFieldName || 'consent',
      text: '',
    },
    ...
    routing: definition.routing || {
      leadType: formType,
      routingMode: 'manual_review',
      recipientGroupRef: definition.leadRecipientRef || 'STARTER_LEAD_RECIPIENT',
    },
    rateLimit: { ...defaultRateLimit, ...definition.rateLimit },
    isActive: definition.isActive ?? true,
    createdAt: definition.createdAt || now,
    updatedAt: definition.updatedAt || now,
    createdBy: definition.createdBy || 'starter-admin',
    updatedBy: definition.updatedBy || 'starter-admin',
  };
}
...
function normalizeField(field: FormFieldDefinition): FormFieldDefinition {
  ...
  return {
    ...field,
    id: field.id || toFieldName(field.name || field.label || 'field'),
...
function getFormDefinitionId(definition: FormDefinition) {
  return definition.formDefinitionId || definition.id || definition.formKey || 'new-form';
}
